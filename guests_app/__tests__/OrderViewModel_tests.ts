...
describe('constructor tests', () => {
  ...
  beforeEach(() => {
   ...
   mockGetMyOrders.mockImplementation(() => mockMakePromise(orderAtServer));
  });
	...
	it('Initializing order to the order in the server', async () => {
		...
		const orderViewModel = new OrderViewModel(requests);
		await flushPromises();
		expect(orderViewModel.getOrder() != null && orderViewModel.getOrder()?.id == order1.id).toBeTruthy();
	});
...
describe('create order tests', () => {
   ...
   it('create order succes', async () => {
		  ...
		  expect(resOrder != null && (await resOrder).id == createdOrder.id).toBeTruthy();
	  });
...
describe('submit review tests', () => {
	...
	it('submit review sucess when order status is arrived', async () => {
		...
		await orderViewModel.submitReview('good service', 5).then(() => is_success = true);
	...
	it('submit review fail when order doesnt exists', async () => {
		...
		await flushPromises();
		let is_fail = false;
		await orderViewModel.submitReview('good service', 5).catch(() => is_fail = true)
