...
export default class OrderViewModel {
	...
	createOrder(items: Map<string, Number>): Promise<Order> {
		...
		return new Promise((_resolve, reject) => reject("createOrder called when order already exists"));
	}
	...
	{	
		...
		return new Promise((_resolve, reject) => reject("cancelOrder called when order doesn't exists"));
	}
	...
	submitReview(deatils: String, rating: Number): Promise<void> {
		...
		if(order != null)
		{
			...
			return new Promise((_resolve, reject) => reject("cancelOrder called when order status isn't arrived"));
		}
		return new Promise((_resolve, reject) => reject("cancelOrder called when order doesn't exists"));
	}
