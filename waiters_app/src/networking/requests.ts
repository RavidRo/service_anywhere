...
function getGuestLocation(orderID: string): Promise<Location> {
...
function getWaiterOrders(waiterID: string): Promise<OrderApi[]> {
	return get<OrderApi[]>('getWaiterOrders', {waiterID});
}

function orderArrived(orderID: string): Promise<void> {
	return post<void>('orderArrived', {orderID});
}

function connectWaiter(): Promise<string> {
